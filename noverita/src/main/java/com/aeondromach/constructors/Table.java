...
public class Table {
    ...
    public Table(IdClassList.IdType map, String title, String type, BiConsumer<String, AnchorPane> singleClick, BiConsumer<String, AnchorPane> doubleClick, String searchTerm, IdClassList.IdType parentMap) {
        this.doubleClick = doubleClick;
        this.map = IdClassList.getIdMap(map);
        this.parentMap = IdClassList.getIdMap(parentMap);
        this.singleClick = singleClick;
        this.title = title;
        this.type = type;
    ...
    private ArrayList<String> parseXml(Document doc) {
        String parentIds = "";
        if (parentMap != null) {
            Document parentDoc = XmlParser.check(searchTerm, parentMap);
            Elements parentElements = parentDoc.select("element[id]");
            for (Element parentElement: parentElements) {
                if (parentElement.attr("id").toLowerCase().equals(searchTerm.toLowerCase())) {
                    parentIds = parentElement.attr("children");
                }
            }
        }
        ...
        for (Element element: elements) {
            if (searchTerm == null) {
                ...
                if (XmlParser.isChildOrParent(element.attr("id"), parentIds) && element.attr("type").toLowerCase().equals(type.toLowerCase()) && !element.attr("name").isEmpty()) {
                    this.hasParent = true;
                    return setElement(arrayList, element);
                }
                if (XmlParser.isChildOrParent(searchTerm, element.attr("parents")) && element.attr("type").toLowerCase().equals(type.toLowerCase()) && !element.attr("name").isEmpty()) {
                    this.hasParent = true;
                    return setElement(arrayList, element);
                }
            }
